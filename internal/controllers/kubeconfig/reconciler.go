...
func (r *reconciler) provisionKubeconfig() *state {
	return &state{
		...
		Transition: func(
			...
			// Refresh the kubeconfig token if it expires in less than 20% of the full token TTL.
			refreshThreshold := time.Duration(float64(ttlDuration) * 0.20)

			if kubeconfig.Status.ServiceAccountTokenExpiration != nil {
				remaining := time.Until(kubeconfig.Status.ServiceAccountTokenExpiration.Time)
			...
			tokenResp, err := r.kubeClient.CoreV1().ServiceAccounts(namespace).CreateToken(ctx, saName, tokenRequest, metav1.CreateOptions{})
			if err != nil {
				return nil, types.ErrorResultf("failed to request token for service account %s/%s: %v", namespace, saName, err)
			}
			...
			tokenExpirationTimestamp := tokenResp.Status.ExpirationTimestamp
			tokenRefreshTimestamp := metav1.NewTime(tokenExpirationTimestamp.Time.Add(-refreshThreshold))
			kubeconfig.Status.KubeconfigSecretRef = ptr.To(secretName)
			kubeconfig.Status.ServiceAccountTokenExpiration = ptr.To(tokenExpirationTimestamp)
			kubeconfig.Status.ServiceAccountTokenRefresh = ptr.To(tokenRefreshTimestamp)
